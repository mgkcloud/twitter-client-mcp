import { Scraper, Profile, Tweet, SearchMode } from 'agent-twitter-client';
...
export class TwitterIntegration {
   ...
   * Search for tweets.
   */
  async searchTweets(query: string, count: number = 20, searchMode: SearchMode = SearchMode.Top): Promise<Tweet[]> {
    await this.ensureAuthenticated();
   ...
   */
  async getConversationThread(tweetId: string, count: number = 20): Promise<Tweet[]> {
    await this.ensureAuthenticated();
   ...
   */
  async uploadMedia(mediaData: Buffer | string, mediaType: string): Promise<any> {
    await this.ensureAuthenticated();
